...
exports.morph = exports.compile = void 0;
const parse_svg_path_1 = __importDefault(require("parse-svg-path"));
...
const compile = (paths) => {
    ...
    const parsedPaths = paths.map(parse_svg_path_1.default); // [Path] -> [Command] -> [Type, param1, param2, ...]
    ...
    const average = fillArray(commands.length, () => []); // The average of all parameters for each command
    const diffs = fillArray(commands.length, () => []); // The difference between each path and the average for each command
    ...
    for (let c = 0; c < commands.length; c++) {
        ...
        for (let p = 0; p < nPaths; p++) {
            for (let v = 0; v < nValues; v++) {
                const param = parsedPaths[p][c][v + 1]; // Invalid paths aren't allowed by parse-svg-path
